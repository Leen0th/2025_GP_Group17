...
struct EditProfileView: View {
    ...
    var body: some View { ZStack { Color.white.ignoresSafeArea(); ScrollView { VStack(alignment: .leading, spacing: 20) { header; profilePictureSection; Divider().padding(.horizontal); formFields; togglesSection; Spacer(minLength: 20); updateButton.padding(.horizontal).padding(.bottom) } } }.navigationBarBackButtonHidden(true).onChange(of: selectedPhotoItem) { _, n in Task { if let d = try? await n?.loadTransferable(type: Data.self) { DispatchQueue.main.async { userProfile.profileImage = UIImage(data: d) } } } } }
