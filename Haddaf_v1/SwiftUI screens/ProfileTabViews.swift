...
struct PlayerProfileContentView: View {
    ...
    @State private var posts: [Post] = [
        Post(
            ...
            likeCount: 350, isLikedByUser: true,
            comments: [
        ...
        Post(
            ...
            authorName: "SALEM AL-DAWSARI", authorImageName: "salem_al-dawsari",
            likeCount: 200, isLikedByUser: false,
            comments: [
...
struct StatsGridView: View {
    ...
    // The stats are separated into two groups
    ...
    private var contactStats: [PlayerStat] {
        [
            .init(title: "Email", value: userProfile.email),
            .init(title: "Phone", value: userProfile.phoneNumber)
        ]
    }
    ...
    // This is the corrected helper view that centers everything
    private func statItemView(for stat: PlayerStat) -> some View {
        VStack(alignment: .center, spacing: 4) { // <-- Hardcoded to .center
            Text(stat.title)
                .font(.caption)
            ...
            Text(stat.value)
                .font(.headline)
                .fontWeight(.semibold)
                .multilineTextAlignment(.center) // <-- Ensures wrapping text is centered
        }
        .frame(maxWidth: .infinity, alignment: .center) // The block is always centered
    }
}

// Reusable view for displaying a single stat item
fileprivate struct StatItemView: View {
    let stat: PlayerStat
    let accentColor: Color
    
    var body: some View {
        // This VStack contains the title and value. Its contents are left-aligned.
        ...
        // This frame modifier centers the entire VStack within its available grid space.
